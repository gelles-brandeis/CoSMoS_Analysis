function pc=SmoothBackground_v3(Bkgndaoifits, SGsmooth, Refaoifits,varargin)
%
% function SmoothBackground_v3(bkgndaoifits, SGsmooth, Refaoifits, ,<parenthandles>)
%
...
% parenthandles== A handles structure for the imscroll gui ('handles' w/in
%              imscroll, and typically used as 'parenthandles' w/in plotargout gui)
%              e.g. contains the AllSpots field (i.e. parenthandles.AllSpots)
%               we will be using parenthandles.AllSpots to insure that none
%               of the background AOIs are close to a detected spot
...
% v3 This version differs from version 2 in that we use AllSpots to detect
%   whether any detected spot is
%   close to one of our background AOIs.  If there is a nearby spot we do
%   not use that AOI in calculating our background (AOI is neglected only
%   for those frames during which a spot is nearby)
...
if inlength>0
    parenthandles=varargin{1}(:);        % parenthandles.Allspots will be used later to insure that none
                                         % of the background AOIs are too close to a detected spot
...
for indxroot=AOInums'
    ...
    if rose>0
        ...
        %keyboard
        FrameRange=parenthandles.AllSpots.FrameVector;    % 1 x N 
        [frmrose  frmcol]=size(FrameRange);     % frmrose=1, frmcol=number of frames in which AllSpots detected spots
        AOISpotnoSpot=zeros(frmcol,3,rose);      % (# of AllSpot frms) x 3 x (# of bkgnd AOIs for current refAOI)
                                     % [frm#   aoi#  0/1=(MT AOI)/(nearby spot)]
             % AOISpotnoSpot indicates for each frame whether each 
             % backtground AOI is close to a detected spot 
        
        for indx=1:rose
            
              % Cycle through the AOIs in our Bkgndaoiinfo2 list, populate AOISpotnoSpot matrix       
            AOISpotnoSpot(:,2,indx)=FrameRange';    % second column is the frame number
            AOInum=Bkgndaoiinfo2(indx,6);
            AOISpotnoSpot(:,1,indx)=AOInum*ones(frmcol,1);   % first column is the AOI number
            radius=4.5;                 % Detecting whether detected spots are within 4.5 pixels of background AOI center
            radius_hys=1;               % No hysterisis in spot detection
                % Third column will be 0/1=(MT AOI)/(nearby spot)
                %keyboard
                % AOISpotLanding will tell us whether any detected spot is within a distance 
                % of 'radius' pixels from one of our background AOIs
            AOISpotnoSpot(:,2:3,indx)=AOISpotLanding(AOInum,radius,parenthandles,Bkgndaoiinfo2,radius_hys);
        ...
        for indx=1:rosedat
           ...
           framenumber=dat(indx,2,1);      % The frame number for this cycle
           %logik=abs(dat(indx,8,:)-SmoothedAveTrace(indx))<4*MeanStd;   % Retain bkgnd values only within 4*MeanStd of smoothed background
           logik1=AOISpotnoSpot(:,2,1)==framenumber;    % Identifies the row for this frame number (for this cycle through the loop)
           logik2=AOISpotnoSpot(logik1,3,:)==0;         % True if there are no nearby spots for this frame
                                             % logik2 picks out those AOIs that do not have any nearby spots for this frame 
           ...
           logik4=logik & logik2;       %  Retain bkgnd values only within 4*MinSDBkTraceMunusMedianTrace of smoothed background
                                        % AND if there are no nearby spots
           %if sum(logik)>0
           if sum(logik4)>0
           ...
           AveBkgndTraceNoOuts(indx,:)=mean(dat(indx,8,logik4));     % Form ave of all the bkgnd traces (having 
