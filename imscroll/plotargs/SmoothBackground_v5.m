...
for indxroot=AOInums'
    ...
    if rose>0
        ...
        AveBkgndData=dat(:,:,1);       % Create data matrix containing list of frame numbers and data
            ...
            % Next test of if rose==1 added by Debi b/c having a single bkgnd AOI
            % did not work
        if rose==1
            stdDev=zeros(size(datonly));
            MedianFrm=datonly;
        else
        ...
        %keyboard
        [roseSMT colSMT]=size(SmoothedMedianTrace);
        if roseSMT>colSMT
                % some sgolay() versions transpose the output
                % This corrects this bug, insuring that  
                % SmoothedMedianTrace is a row vector
            SmoothedMedianTrace=SmoothedMedianTrace';
        end
        ...
        for indx4=1:rose
            ...
            % ******** version 19b we changed SmoothedMedianTrace to SmoothedMedTrace'
            % Looks as though the vector matrix was transposed in new
            % version sgolay( ) above ?
            %keyboard
